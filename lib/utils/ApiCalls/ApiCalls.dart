...
class ApiCalls {
  static var customerGuid ;
  static final String cookie = '.Nop.Customer=' + ConstantsVar.prefs.getString(kguidKey)!??'';
  static final header = {'Cookie': '.Nop.Customer=' + ConstantsVar.prefs.getString(kguidKey)!??''};
  ...
  static Future<String> addToCart({
    ...
    final uri = Uri.parse(
        '${baseUrl}AddToCart?apiToken=${ConstantsVar.prefs.getString(kapiTokenKey)}&customerid=${ConstantsVar.prefs.getString(kcustomerIdKey)}&productid=$productId&itemquantity=1&$kstoreIdVar$storeId&'
  ...
  static Future<bool> addToWishlist({
    ...
    final url = Uri.parse(
      '${baseUrl}AddToWishlist?apiToken=${ConstantsVar.prefs.getString(kapiTokenKey)}&customerid=${ConstantsVar.prefs.getString(kcustomerIdKey)}&productid=$productId&itemquantity=1&recipientName=$receiverName&recipientEmail=$receiverEmail&senderName=$senderName&senderEmail=$senderEmail&giftCardMessage=$msg&selectedAttributeId=$attributeId,$kstoreIdVar$storeId',
    );
...
/*Show Billing Address*/
  ...
  static Future<String> getShippingMethods() async {
    ...
    try {
      ...
      var response = await http.get(uri, headers: {'Cookie': '.Nop.Customer=' + ConstantsVar.prefs.getString(kguidKey)!??''}).timeout(
        const Duration(seconds: 15),
        onTimeout: () {
          Fluttertoast.showToast(msg: "Connection Timeout.\nPlease try again.");
          // Time has run out, do what you wanted to do.
